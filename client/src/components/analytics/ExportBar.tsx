...
interface ExportBarProps {
  ...
  disabled?: boolean;
  /**
   * Phone composition (F-11): ONE "Export" button whose popover holds both
   * exports — the current view and the date range — instead of two buttons
   * in the header. The popover keeps its test ids, so the f8 "CSV export
   * popover opens" contract holds on every width.
   */
  compact?: boolean;
}
 ...
 * is invalid (P1-18).
 */
export function ExportBar({ onExportView, onExportRange, defaultFrom, defaultTo, disabled, compact = false }: ExportBarProps) {
    ...
    return (
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          <Button type="button" variant="outline" size="sm" disabled={disabled} data-testid="button-export" className="min-h-11">
            <Download aria-hidden="true" strokeWidth={1.75} />
            {t("analyticsV2.export.button")}
          </Button>
        </PopoverTrigger>
        <PopoverContent align="end" className="w-[calc(100vw-2rem)] max-w-xs space-y-3" data-testid="popover-export-range">
          <Button type="button" variant="outline" size="sm" className="min-h-11 w-full" onClick={handleViewExport} data-testid="button-export-view">
            <Download aria-hidden="true" strokeWidth={1.75} />
            {t("analytics.exportView")}
          </Button>
          <div className="border-t border-border pt-3">
            <p className="text-body-sm font-medium text-foreground">{t("analytics.exportRange")}</p>
          </div>
          {rangeForm}
        </PopoverContent>
      </Popover>
