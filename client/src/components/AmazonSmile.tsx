...
interface AmazonLogoProps {
  className?: string;
  size?: "sm" | "md" | "lg";
  /** Accessible name; omit for a decorative mark next to visible text. */
  title?: string;
  /** On ink / navy surfaces the "a" is white so the mark stays legible. */
  onDark?: boolean;
}

/**
 * Amazon "a" with the smile — the only mark the header uses. Decorative by
 * default (`aria-hidden`); pass `title` when it stands alone. Never flipped in
 * RTL (logos keep their orientation).
 */
export function AmazonLogo({ className, size = "md", title, onDark = false }: AmazonLogoProps) {
  const sizes = {
    sm: "size-6",
    md: "size-8",
    lg: "size-12",
