      ...
      {availabilityQuery.isLoading ? (
        ...
        <ul className="space-y-3">
          {MONDAY_FIRST.map((day, index) => {
            ...
            return (
              <li key={day} className="rounded-lg border border-border bg-card p-4" data-testid={`day-${day}`}>
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <h2 id={headingId} className="text-body font-medium text-foreground">
                    {dayNames[index]}
                  </h2>
                  <Button variant="outline" size="sm" onClick={() => addSlot(day)} data-testid={`button-add-slot-${day}`}>
                    <Plus aria-hidden="true" />
                    {t("dashboardV2.availability.addWindow")}
                  </Button>
                </div>
                {daySlots.length === 0 ? (
                  <p className="mt-2 text-body-sm text-muted-foreground">{t("dashboardV2.availability.noWindows")}</p>
                ) : (
                  <ul className="mt-3 space-y-2" aria-labelledby={headingId}>
                    {daySlots.map((slot) => {
                      ...
                      return (
                        <li
                          key={slot.id}
                          className={cn("flex flex-wrap items-center gap-3 rounded-lg border border-border p-3", invalid && "border-destructive")}
                        ...
                        >
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <Select value={slot.start_time} onValueChange={(value) => updateSlot(slot.id, { start_time: value })}>
                              <SelectTrigger id={`${ids}-start-${slot.id}`} className="w-28" aria-invalid={invalid} data-testid={`select-start-${slot.id}`}>
                                <SelectValue />
                              </SelectTrigger>
                              <SelectContent>
                                {HOURS.map((h) => (
                            ...
                            <Select value={slot.end_time} onValueChange={(value) => updateSlot(slot.id, { end_time: value })}>
                              <SelectTrigger id={`${ids}-end-${slot.id}`} className="w-28" aria-invalid={invalid} data-testid={`select-end-${slot.id}`}>
                                <SelectValue />
                              </SelectTrigger>
                              <SelectContent>
                                {HOURS.map((h) => (
