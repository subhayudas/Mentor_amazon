import * as React from "react";
import { Link } from "wouter";
import { useTranslation } from "react-i18next";
import { CalendarDays, CalendarClock, Check, CheckCircle2, Share2, Video, X } from "lucide-react";
import { toast } from "sonner";

import { CalEmbed } from "@/components/CalEmbed";
import { DashboardHeader, DashboardShell, useDashboardIdentity } from "@/components/dashboard/DashboardShell";
import { Badge } from "@/components/ui/badge";
import { StatusBadge } from "@/components/StatusBadge";
import { FEATURED_MENTORS } from "@/data/featuredMentors";
import { logActivity } from "@/lib/activity";
import type { Booking } from "@/lib/database";
import { DEFAULT_CAL_LINK, normalizeCalLink } from "@/lib/demo";
import { localStore } from "@/lib/localStore";
import { ROUTES } from "@/lib/routes";
import { cn } from "@/lib/utils";
import { UPCOMING_STATUSES, useDashboardData } from "@/pages/dashboard/data";

/**
 * Bookings `/dashboard/bookings` (Figma "Bookings" page): Requests /
 * Upcoming / Completed / Cancelled and the session rows. Mentors accept or
 * decline requests, mark sessions completed and cancel; mentees cancel and
 * reschedule (the mentor's Cal.com calendar opens to pick the new time).
 * Every action is appended to the activity feed.
 */
type Tab = "requests" | "upcoming" | "completed" | "canceled";

export default function DashboardBookings() {
  const { t, i18n } = useTranslation();
  const lang = i18n.language;
  const { bookings, mentees, mentors, demo, role, profileId } = useDashboardData();
  const { displayName, signedIn } = useDashboardIdentity();
  const isMentee = role === "mentee";
  const isMentor = role === "mentor";
  const isAdmin = role === "admin";
  const pendingCount = bookings.filter((b) => b.status === "pending").length;
  const [tab, setTab] = React.useState<Tab>(() => (pendingCount > 0 ? "requests" : "upcoming"));
  const [reschedule, setReschedule] = React.useState<Booking | null>(null);

  const menteeById = React.useMemo(() => new Map(mentees.map((m) => [m.id, m])), [mentees]);
  const mentorById = React.useMemo(() => new Map(mentors.map((m) => [m.id, m])), [mentors]);
  const featuredById = React.useMemo(() => new Map(FEATURED_MENTORS.map((m) => [m.id, m])), []);
  const mentorFor = (id: string) => mentorById.get(id) ?? featuredById.get(id);
  const now = Date.now();
  const rows = React.useMemo(() => {
    const list =
      tab === "requests"
        ? bookings.filter((b) => b.status === "pending")
        : tab === "upcoming"
          ? bookings.filter((b) => UPCOMING_STATUSES.includes(b.status) && (!b.scheduled_at || new Date(b.scheduled_at).getTime() > now))
          : tab === "completed"
            ? bookings.filter((b) => b.status === "completed")
            : bookings.filter((b) => b.status === "canceled" || b.status === "rejected");
    return list
      .slice()
      .sort((a, b) => {
        const ta = new Date(a.scheduled_at ?? a.completed_at ?? a.created_at).getTime();
        const tb = new Date(b.scheduled_at ?? b.completed_at ?? b.created_at).getTime();
        return tab === "upcoming" || tab === "requests" ? ta - tb : tb - ta;
      })
      .slice(0, 40);
  }, [bookings, tab, now]);
  const when = React.useMemo(() => new Intl.DateTimeFormat(lang, { weekday: "short", day: "numeric", month: "short", hour: "numeric", minute: "2-digit" }), [lang]);

  const actor = () => ({ actor_type: (isAdmin ? "admin" : isMentee ? "mentee" : "mentor") as "admin" | "mentee" | "mentor", actor_id: profileId ?? undefined, actor_name: displayName });
  const nameOf = (b: Booking) => ({ mentee: menteeById.get(b.mentee_id)?.name ?? t("showcase.bookings.mentee"), mentor: mentorFor(b.mentor_id)?.name ?? t("showcase.bookings.mentor") });

  const decide = (b: Booking, status: "accepted" | "rejected") => {
    localStore.update("bookings", b.id, { status, responded_at: new Date().toISOString() });
    logActivity({ ...actor(), type: status === "accepted" ? "request_accepted" : "request_declined", subject_type: "booking", subject_id: b.id, visible_to: [b.mentor_id, b.mentee_id], summary: t(status === "accepted" ? "showcase.activity.summaries.requestAccepted" : "showcase.activity.summaries.requestDeclined", { mentee: nameOf(b).mentee }) });
    toast.success(t(status === "accepted" ? "showcase.bookings.toast.accepted" : "showcase.bookings.toast.declined"));
  };
  const cancel = (b: Booking) => {
    localStore.update("bookings", b.id, { status: "canceled", canceled_at: new Date().toISOString() });
    logActivity({ ...actor(), type: "booking_canceled", subject_type: "booking", subject_id: b.id, visible_to: [b.mentor_id, b.mentee_id], summary: t("showcase.activity.summaries.canceled", { mentor: nameOf(b).mentor, mentee: nameOf(b).mentee }) });
    toast.success(t("showcase.bookings.toast.canceled"));
    setTab("canceled");
  };
  const complete = (b: Booking) => {
    localStore.update("bookings", b.id, { status: "completed", completed_at: new Date().toISOString(), session_duration_minutes: b.session_duration_minutes ?? 30 });
    logActivity({ ...actor(), type: "session_completed", subject_type: "booking", subject_id: b.id, visible_to: [b.mentor_id, b.mentee_id], summary: t("showcase.activity.summaries.completed", { mentee: nameOf(b).mentee }) });
    toast.success(t("showcase.bookings.toast.completed"));
    setTab("completed");
  };
  const onRescheduled = (b: Booking) => (detail: { startTime?: string; uid?: string }) => {
    localStore.update("bookings", b.id, { status: "confirmed", scheduled_at: detail.startTime ?? b.scheduled_at, cal_event_uri: detail.uid ?? b.cal_event_uri });
    logActivity({ ...actor(), type: "booking_rescheduled", subject_type: "booking", subject_id: b.id, visible_to: [b.mentor_id, b.mentee_id], summary: t("showcase.activity.summaries.rescheduled", { mentor: nameOf(b).mentor }) });
    toast.success(t("showcase.bookings.toast.rescheduled"));
    setReschedule(null);
  };
  const calLinkFor = (b: Booking) => {
    const own = normalizeCalLink(mentorFor(b.mentor_id)?.cal_link);
    const shared = normalizeCalLink(DEFAULT_CAL_LINK);
    return own || (shared ? `${shared}${shared.includes("/") ? "" : "/30min"}` : "");
  };

  const tabCls = (active: boolean) => cn("relative h-11 px-1 text-[16px] transition-colors duration-fast", active ? "font-bold text-[var(--sc-ink)] after:absolute after:inset-x-0 after:bottom-0 after:h-0.5 after:bg-[var(--sc-ink)]" : "text-[#6c6c84] hover:text-[var(--sc-ink)]");
  const btnPrimary = "inline-flex h-9 items-center gap-1.5 rounded-[6px] bg-[var(--sc-ink)] px-3 text-[13px] font-bold text-white hover:bg-black";
  const btnOutline = "inline-flex h-9 items-center gap-1.5 rounded-[6px] border border-[#d9d9d9] px-3 text-[13px] font-semibold text-[var(--sc-ink)] hover:bg-[var(--sc-sand)]";

  const emptyTitle = tab === "requests" ? t("showcase.bookings.empty.requests") : tab === "completed" ? t("showcase.bookings.empty.completed") : tab === "canceled" ? t("showcase.bookings.empty.canceled") : isMentee ? t("showcase.dashboard.menteeEmptyTitle") : t("showcase.bookings.emptyTitle");
  const emptyBody = tab === "upcoming" ? (isMentee ? t("showcase.dashboard.menteeEmptyBody") : t("showcase.bookings.emptyBody")) : t("showcase.bookings.empty.body");

  return (
    <DashboardShell active="bookings">
      <DashboardHeader title={t(isMentee ? "showcase.analytics.nav.mySessions" : "showcase.analytics.nav.bookings")} trailing={demo ? <Badge tone="warning">{t("analyticsV2.demoBadge")}</Badge> : undefined} />
      <div className="px-4 py-6 sm:px-8 lg:px-12">
        <div role="tablist" className="flex flex-wrap gap-6 border-b border-[var(--sc-hairline)] sm:gap-8">
          {(["requests", "upcoming", "completed", "canceled"] as Tab[]).map((k) => (
            <button key={k} type="button" role="tab" aria-selected={tab === k} className={tabCls(tab === k)} onClick={() => setTab(k)} data-testid={`tab-${k}`}>
              {t(`showcase.bookings.${k}`)}
              {k === "requests" && pendingCount > 0 && <span className="ms-2 inline-flex min-w-5 items-center justify-center rounded-full bg-[var(--sc-ink)] px-1.5 text-[11px] font-bold text-white">{pendingCount}</span>}
            </button>
          ))}
        </div>

        {rows.length === 0 ? (
          <div className="mx-auto max-w-[420px] py-16 text-center">
            <div className="mx-auto grid size-[120px] place-items-center rounded-full bg-[#ffd23f]/60" aria-hidden="true">
              {tab === "upcoming" && !isMentee ? <Share2 className="size-12 text-[var(--sc-ink)]" strokeWidth={1.25} /> : <CalendarClock className="size-12 text-[var(--sc-ink)]" strokeWidth={1.25} />}
            </div>
            <h2 className="mt-8 text-[22px] font-bold text-[var(--sc-ink)]">{emptyTitle}</h2>
            <p className="mt-2 text-[15px] text-[#6c6c84]">{emptyBody}</p>
            {tab === "upcoming" && (
              <Link href={isMentee || isAdmin ? ROUTES.mentors : "/dashboard/profile"} className="mt-6 inline-flex h-11 items-center rounded-[8px] bg-[var(--sc-ink)] px-5 text-[14px] font-bold text-white hover:bg-black">
                {t(isMentee || isAdmin ? "showcase.hero.cta" : "showcase.bookings.sharePage")}
              </Link>
            )}
          </div>
        ) : (
          <ul className="mt-2 divide-y divide-[var(--sc-hairline)]">
            {rows.map((b: Booking) => {
              const names = nameOf(b);
              const at = b.scheduled_at ?? b.completed_at ?? b.created_at;
              const past = b.scheduled_at ? new Date(b.scheduled_at).getTime() < now : false;
              const open = !["canceled", "completed", "rejected"].includes(b.status);
              const canAct = signedIn;
              return (
                <li key={b.id} className="flex flex-wrap items-center gap-4 py-4" data-testid={`booking-row-${b.id}`}>
                  <span className="inline-flex size-12 shrink-0 items-center justify-center rounded-full bg-[var(--sc-peach)] text-[15px] font-bold text-[var(--sc-ink)]" aria-hidden="true">
                    {(isMentee ? names.mentor : names.mentee).slice(0, 1)}
                  </span>
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-[15px] font-semibold text-[var(--sc-ink)]">{isMentee ? t("showcase.bookings.with", { name: names.mentor }) : names.mentee}</p>
                    <p className="truncate text-[13px] text-[#6c6c84]">
                      {b.goal ?? t("showcase.bookings.session")}
                      {!isMentee ? ` · ${t("showcase.bookings.with", { name: names.mentor })}` : ""}
                    </p>
                  </div>
                  <p className="inline-flex items-center gap-2 text-[14px] text-[var(--sc-ink)]">
                    <CalendarDays className="size-4 text-[#6c6c84]" aria-hidden="true" />
                    {b.scheduled_at || b.status === "completed" ? when.format(new Date(at)) : t("showcase.bookings.timeTbc")}
                  </p>
                  <p className="inline-flex items-center gap-2 text-[13px] text-[#6c6c84]">
                    <Video className="size-4" aria-hidden="true" />
                    {t("showcase.rail.minutes", { minutes: b.session_duration_minutes ?? 30 })}
                  </p>
                  <StatusBadge status={b.status} />
                  {canAct && (
                    <span className="flex flex-wrap gap-2">
                      {isMentor && b.status === "pending" && (
                        <>
                          <button type="button" onClick={() => decide(b, "accepted")} className={btnPrimary} data-testid={`button-accept-${b.id}`}>
                            <Check className="size-4" aria-hidden="true" />
                            {t("showcase.bookings.accept")}
                          </button>
                          <button type="button" onClick={() => decide(b, "rejected")} className={btnOutline} data-testid={`button-decline-${b.id}`}>
                            <X className="size-4" aria-hidden="true" />
                            {t("showcase.bookings.decline")}
                          </button>
                        </>
                      )}
                      {isMentor && UPCOMING_STATUSES.includes(b.status) && (
                        <button type="button" onClick={() => complete(b)} className={past ? btnPrimary : btnOutline} data-testid={`button-complete-${b.id}`}>
                          <CheckCircle2 className="size-4" aria-hidden="true" />
                          {t("showcase.bookings.markCompleted")}
                        </button>
                      )}
                      {isMentee && UPCOMING_STATUSES.includes(b.status) && calLinkFor(b) && (
                        <button type="button" onClick={() => setReschedule(b)} className={btnOutline} data-testid={`button-reschedule-${b.id}`}>
                          <CalendarClock className="size-4" aria-hidden="true" />
                          {t("showcase.bookings.reschedule")}
                        </button>
                      )}
                      {open && (
                        <button type="button" onClick={() => cancel(b)} className={btnOutline} data-testid={`button-cancel-${b.id}`}>
                          <X className="size-4" aria-hidden="true" />
                          {t("showcase.bookings.cancel")}
                        </button>
                      )}
                    </span>
                  )}
                </li>
              );
            })}
          </ul>
        )}
      </div>

      {reschedule && (
        <CalEmbed
          calLink={calLinkFor(reschedule)}
          mentorName={nameOf(reschedule).mentor}
          menteeName={displayName}
          bookingId={reschedule.id}
          open
          onOpenChange={(o) => {
            if (!o) setReschedule(null);
          }}
          onBookingSuccessful={onRescheduled(reschedule)}
        />
      )}
    </DashboardShell>
  );
}
