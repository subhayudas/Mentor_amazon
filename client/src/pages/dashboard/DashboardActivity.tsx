 ...
 * an admin (or the showcase account) sees everything.
 */
...
export function ActivityList({ events, lang, compact }: { events: ActivityEvent[]; lang: string; compact?: boolean }) {
  ...
  return (
    <ol className={cn("divide-y divide-[var(--sc-hairline)]", compact && "text-[13px]")}>
      {events.map((e) => {
        const Icon = ICONS[e.type] ?? Bell;
        return (
          <li key={e.id} className={cn("flex items-start gap-3", compact ? "py-2.5" : "py-4")}>
            ...
            <div className="min-w-0 flex-1">
              <p className={cn("text-[var(--sc-ink)]", compact ? "text-[13px]" : "text-[15px]")}>{e.summary}</p>
...
export default function DashboardActivity() {
  ...
  const { events, isLoading } = useActivity(user?.profile_id ?? null, { all: !signedIn || user?.user_type === "admin" });
  ...
  return (
    <DashboardShell active="activity">
      ...
      <div className="px-4 py-4 sm:px-8 lg:px-12">
        ...
        {isLoading ? <p className="py-8 text-[14px] text-[#6c6c84]">{t("common.loading")}</p> : <ActivityList events={shown} lang={i18n.language} />}
      </div>
    </DashboardShell>
  );
}
